...
def add_inclusion(request):
    if request.method == 'POST':
        ...
        if participant_form.is_valid():
            ...
            new_url = f'/DataEntry/add_inclusion/{participant_num}/'
...
def add_inclusion_participant(request, participant_num=None):
    participant = None  # Initialize to None initially

    if participant_num is not None:
        # Check if a participant with the given participant_num exists
        participant = get_object_or_404(Participant, participant_number=participant_num)
    ...
    if request.method == 'POST':
        ...
        if participant_form.is_valid():
            ...
            return redirect('add_inclusion_participant', participant_num=participant_num)
    else:
        # Create a new form, either empty or pre-filled with participant data
        participant_form = ParticipantForm(instance=participant)

    context = {
        'participant_form': participant_form,
        'participant': participant,  # You can use this in your template if needed
    }
...
# def add_inclusion(request): not fully working oct 25
#     if request.method == 'POST':
#         participant_form = ParticipantForm(request.POST)
#         inclusion_form_data = inclusion_form(request.POST)
...
#             except ValueError:
#                 return JsonResponse({'error': 'Invalid participant number. Please enter a valid integer.'}, status=400)
...
#     else:
#         participant_form = ParticipantForm()
#         inclusion_form_data = inclusion_form()

#     context = {
#         'participant_form': participant_form,
#         'inclusion_form': inclusion_form_data,
#     }
#     return render(request, 'DataEntry/inclusion.html', context)
...
def add_inclusion_old(request, participant_num=None):
    participant_already_exists = False
...
def add_mandatory_questionaire(request, participant_id=None):
    ...
    if participant_id: 
        ...
        print(participant)
        print('the participant numebr is: ')
...
#old view, witha bug on query sets and nodule isntances
...
#     ct_nodule_form = ct_scan_nodule_1.objects.all()
...
def add_ct_scan(request, participant_id=None):
    print("participant_id received:", participant_id)
    participant = get_object_or_404(Participant, participant_number=participant_id)
    print("Participant object retrieved:", participant)
